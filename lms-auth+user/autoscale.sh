...
get_cpu_usage() {
  docker stats --no-stream --format "{{.CPUPerc}}" $(docker ps -q -f "name=auth_stack_lmsauthusers") | sed 's/%//'
}

# Function to get the current memory usage of the service
get_mem_usage() {
  docker stats --no-stream --format "{{.MemPerc}}" $(docker ps -q -f "name=auth_stack_lmsauthusers") | sed 's/%//'
}
