...
public class BookServiceWBTest {
    ...
    @Test
    public void testGetBooksSuggestionsForReader_NoInterest() {
        ...
        when(readerDetails.getInterestList()).thenReturn(List.of());

        assertThrows(NotFoundException.class, () -> bookService.getBooksSuggestionsForReader(readerNumber));
    }

    @Test
    public void testFindTop5BooksLent() {
        LocalDate oneYearAgo = LocalDate.now().minusYears(1);
        Pageable pageableRules = PageRequest.of(0, 5); // Use a direct PageRequest instance

        // Mock the list of BookCountDTO
        List<BookCountDTO> bookList = List.of(mock(BookCountDTO.class));
        Page<BookCountDTO> bookCountDTOPageMock = new PageImpl<>(bookList);

        // Stub the repository call with the exact arguments
        when(bookRepository.findTop5BooksLent(oneYearAgo, pageableRules)).thenReturn(bookCountDTOPageMock);
        ...
        verify(bookRepository, times(1)).findTop5BooksLent(oneYearAgo, pageableRules);
    }
