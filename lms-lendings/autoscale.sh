...
scale_db() {
  ...
  docker service scale lendings_stack_h2=$replicas
}

# Function to scale the MS application service up or down
scale_service() {
  local replicas=$1
  echo "Scaling Microservice LMS-Lendings to $replicas replicas..."
  docker service scale lendings_stack_lmslendings=$replicas
}
