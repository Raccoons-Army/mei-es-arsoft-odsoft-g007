...
public class RecommendationsCDCConsumerIT {
    ...
    void testMessageProcessing() throws Exception {
        ...
        File pactFile = new File("target/pacts/recommendation_created-consumer-recommendation_event-producer.json");
        ...
        for (Message messageGeneratedByPact : messagesGeneratedByPact) {
            ...
            assertDoesNotThrow(() -> {
                listener.receiveRecommendationCreated(messageToBeSentByRabbit);
            });

            // somehow optional: verify interactions with the mocked service
            verify(recommendationService, times(1)).create(any(RecommendationViewAMQP.class));
        }
    }
}