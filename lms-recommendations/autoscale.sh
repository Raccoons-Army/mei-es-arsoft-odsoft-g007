...
get_cpu_usage() {
  cpu_usage=$(docker stats --no-stream --format "{{.CPUPerc}}" $(docker ps -q -f "name=recommendations_stack_lmsrecommendations") | sed 's/%//' | head -n 1)
...
get_mem_usage() {
  mem_usage=$(docker stats --no-stream --format "{{.MemPerc}}" $(docker ps -q -f "name=recommendations_stack_lmsrecommendations") | sed 's/%//' | head -n 1)
  # Ensure the value is an integer (multiply by 100 if necessary)
