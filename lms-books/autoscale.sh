...
get_cpu_usage() {
  docker stats --no-stream --format "{{.CPUPerc}}" $(docker ps -q -f "name=books_stack_lmsbooks") | sed 's/%//'
}

# Function to get the current memory usage of the service
get_mem_usage() {
  docker stats --no-stream --format "{{.MemPerc}}" $(docker ps -q -f "name=books_stack_lmsbooks") | sed 's/%//'
}
