...
get_cpu_usage() {
  cpu_usage=$(docker stats --no-stream --format "{{.CPUPerc}}" $(docker ps -q -f "name=books_stack_lmsbooks") | sed 's/%//' | head -n 1)
...
get_mem_usage() {
  mem_usage=$(docker stats --no-stream --format "{{.MemPerc}}" $(docker ps -q -f "name=books_stack_lmsbooks") | sed 's/%//' | head -n 1)
  # Ensure the value is an integer (multiply by 100 if necessary)
...
scale_db() {
  ...
  docker service scale books_stack_h2=$replicas
}

# Function to scale the MS application service up or down
scale_service() {
  local replicas=$1
  echo "Scaling Microservice LMS-Books to $replicas replicas..."
  docker service scale books_stack_lmsbooks=$replicas
}
...
CURRENT_REPLICAS=$(get_replicas books_stack_lmsbooks)
